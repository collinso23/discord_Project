...
class DnDScraper():
    ...
    def searchDNDWebsite(self, section, nameOfSearch, specifier=""):
        ...
        rstring = f"Unable to find soup: {nameOfSearch}"
        print(url)
        soup = self.getEntireHTMLPage(url)
        """
        if section == "item":
            soup = self.getEntireHTMLPage(url +
                                          "&f=equipment,magic-items&c=items")
            ...
            soup = self.getEntireHTMLPage(
                url + "&f=backgrounds,classes,feats,races&c=characters")
    ...
    def searchItem(self, soup, kind):
        searchListing = soup.find('div',
                                  {'class': 'ddb-search-results-listing'})
        searches = soup.find_all('div',
                                 {'class': 'ddb-search-results-listing-item'})
        rstring = ""
        for search in searches:
            if search is not None:
                if kind == 1:
                    if search.find('span',
                                   {'class': 'magic-items'}) is not None:
        ...
        for search in searches:
            if search is not None:
                if search.find('span', {'class': 'classes'}) is not None:
                    ...
                    url = "https://www.dndbeyond.com" + link
                    soup = self.getEntireHTMLPage(url)
                    return self.extractInformationFromClassPage(
                        soup, specifier)
                    break
        return "Nothing was caught"
        ...
        for search in searches:
            if search is not None:
                if search.find('span', {'class': 'spells'}) is not None:
                    ...
                    url = "https://www.dndbeyond.com" + link
                    soup = self.getEntireHTMLPage(url)
                    return self.extractInformationFromSpellPage(
                        soup, nameOfSearch)
                    break
        return "Nothing was caught"
    ...
    def extractInformationFromSpellPage(self, soup, nameOfSearch):
        ...
        return self.fixSpacingOnText(
            self.breakTextByLength(mainInformation.text))
